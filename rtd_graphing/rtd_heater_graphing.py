...
for timestamp in sorted(dim.images.keys()):
	...
	if image:
		...
		axis.text(0,1.2,'Bias = ' + str(bias) + 'V',fontsize=12,transform = axis.transAxes)
		axis.text(0.4,1.2,RTD,fontsize=12, transform=axis.transAxes)
		axis.text(1.05,1.000,'Electrical', fontsize=7, transform=axis.transAxes)
		axis.text(1.05,0.950,T1, fontsize=7, transform=axis.transAxes)
		axis.text(1.05,0.900,T2, fontsize=7, transform=axis.transAxes)
		axis.text(1.05,0.850,T3, fontsize=7, transform=axis.transAxes)
		axis.text(1.25,1.000,'Thermographic', fontsize=7, transform=axis.transAxes)
		...
		txt = "{0:.1f}C".format(img[40,(1+1)*pix_pr_mm + dim.left_edge]/scale_temp + min_temp)
		#axis.text(1.25,0.850,str(img[40,(1+1)*pix_pr_mm + dim.left_edge]/scale_temp + min_temp), fontsize=7, transform=axis.transAxes)
		axis.text(1.25,0.850,txt, fontsize=7, transform=axis.transAxes)
		b = plt.Rectangle([1*pix_pr_mm + dim.left_edge,10],1*pix_pr_mm,60,color="m",fill=False)
		fig.gca().add_artist(b)
	
		# Heater 2, 6.6mm from top, 1mm wide
		txt = "{0:.1f}C".format(img[40,6.6*pix_pr_mm + dim.left_edge]/scale_temp + min_temp)
		axis.text(1.25,0.900,txt, fontsize=7, transform=axis.transAxes)
		b = plt.Rectangle([6.6*pix_pr_mm + dim.left_edge,10],1*pix_pr_mm,60,color="b",fill=False)
		fig.gca().add_artist(b)
	
		# Heater 1, 12.9 from top, 2mm wide
		txt = "{0:.1f}C".format(img[40,12.9*pix_pr_mm + dim.left_edge]/scale_temp + min_temp)
		#axis.text(1.25,0.950,str(img[40,12.9*pix_pr_mm + dim.left_edge]/scale_temp + min_temp), fontsize=7, transform=axis.transAxes)
		axis.text(1.25,0.950,txt, fontsize=7, transform=axis.transAxes)
		b = plt.Rectangle([12.9*pix_pr_mm + dim.left_edge,10],2*pix_pr_mm,60,color="r",fill=False)
		fig.gca().add_artist(b)
		...
		delta  = "$\Delta$T = {0:.1f}C".format(max(zoom_data)-min(zoom_data))
		axis.text(0.75,1.2,delta,fontsize=12,transform = axis.transAxes)
		a = plt.axes([.75, .12, .21, .2], axisbg='w')
		...
		b = plt.axes([.75, .4, .21, .2], axisbg='w')
		b.plot(data['T1'][i-3:i+10,0], data['T1'][i-3:i+10,1], 'r-',label="H1")
		b.plot(data['T2'][i-3:i+10,0], data['T2'][i-3:i+10,1], 'b-',label="H2")
		b.plot(data['T3'][i-3:i+10,0], data['T3'][i-3:i+10,1], 'm-',label="H3")
		b.plot(data['RTD_temp'][i-3:i+10,0], data['RTD_temp'][i-3:i+10,1], 'c-',label="RTD")
		b.vlines(timestamp,b.get_ylim()[0],b.get_ylim()[1],linestyles='dotted')
		b.tick_params(direction='in', length=3, width=1, colors='k',labelsize=6,axis='both',pad=3)
		b.set_xlabel('Time / s', fontsize=5)
		...
		axis.tick_params(direction='in', length=3, width=1, colors='k',labelsize=6,axis='both',pad=3)
